...
def init_Classifier(net, gpu_ids=[]):
    if len(gpu_ids) > 0:
        assert(torch.cuda.is_available())
        net.to(gpu_ids[0])
        net = torch.nn.DataParallel(net, gpu_ids)
...
def define_C(num_classes, resnet_shortcut, sample_size1, sample_size2, sample_duration, netC, last_fc=True, gpu_ids=[]):
    net = None
    if netC == 'resnet_10':
        net = ResNet(BasicBlock, [1, 1, 1, 1], sample_size1, sample_size2, sample_duration, resnet_shortcut, num_classes, last_fc)
    if netC == 'resnet_18':
        net = ResNet(BasicBlock, [2, 2, 2, 2], sample_size1, sample_size2, sample_duration, resnet_shortcut, num_classes, last_fc)
    if netC == 'resnet_34':
        net = ResNet(BasicBlock, [3, 4, 6, 3], sample_size1, sample_size2, sample_duration, resnet_shortcut, num_classes, last_fc)
    if netC == 'ALexNet':
        net = ALexNet(num_classes)
    if netC == '3DCNN':
        net = _3D_CNN(num_classes)
    if netC == 'CNN_3D':
        net = CNN_3D(num_classes)
    return init_Classifier(net, gpu_ids)
...
class CNN_3D(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv1 = nn.Conv3d(2, 64, kernel_size=4, stride=2, padding=1)
        ...
        self.conv4 = nn.Conv3d(256, 512, kernel_size=4, stride=1, padding=1, bias=False)
        self.BN3d1 = nn.BatchNorm3d(num_features=64)
        self.BN3d2 = nn.BatchNorm3d(num_features=128)
        self.BN3d3 = nn.BatchNorm3d(num_features=256)
        self.BN3d4 = nn.BatchNorm3d(num_features=512)
        self.LeakReLU = nn.LeakyReLU(0.2, True)
        self.pool = nn.MaxPool3d(kernel_size=2, stride=2)
        self.fc1 = nn.Linear(512 * 3 * 1 * 1, 512)
        self.dropout = nn.Dropout(p=0.5)
        self.fc2 = nn.Linear(512, num_classes)

    def forward(self, x):
        x = self.LeakReLU(self.BN3d1(self.conv1(x)))
        x = self.pool(x)
        x = self.LeakReLU(self.BN3d2(self.conv2(x)))
        x = self.pool(x)
        x = self.LeakReLU(self.BN3d3(self.conv3(x)))
        x = self.pool(x)
        x = self.LeakReLU(self.BN3d4(self.conv4(x)))
        x = self.pool(x)
        x = x.view(x.size(0), -1)
        x = self.fc1(x)
        x = self.dropout(x)
...
class ALexNet(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.conv1 = nn.Conv3d(2, 96, kernel_size=11, stride=4, padding=0)
        self.conv2 = nn.Conv3d(96, 256, kernel_size=5, stride=1, padding=2)
        self.conv3 = nn.Conv3d(256, 384, kernel_size=3, stride=1, padding=1)
        self.conv4 = nn.Conv3d(384, 384, kernel_size=3, stride=1, padding=1)
        self.conv5 = nn.Conv3d(384, 256, kernel_size=3, stride=1, padding=1)
        self.pool = nn.MaxPool3d(kernel_size=3, stride=2)
        self.fc1 = nn.Linear(256*14*6*6, 4096)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, sample_size1, sample_size2, sample_duration, shortcut_type='B', num_classes=400, last_fc=True):
